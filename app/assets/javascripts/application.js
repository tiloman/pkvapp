...
document.addEventListener("turbolinks:load", function () {
  ...
  function update_values() {
    ...
    var insurance_payback = $(
      "#best_in_place_operation_" + operation_id + "_insurance_payback"
    ).html();
    insurance_payback = parseInt(insurance_payback.replace(/,/g, ""));
    var assistance_ratio = $("#assistance_ratio").html();
    var assistance_payback = $(
      "#best_in_place_operation_" + operation_id + "_assistance_payback"
    ).html();
    ...
    $("#insurance_difference").html(
      (
        Math.round(
          ((insurance_payback - value * insurance_ratio) / 100) * 100
        ) / 100
      ).toFixed(2)
    );
    $("#insurance_calculated").html(
      Math.round(value * (insurance_ratio / 100)).toFixed(2)
    );
    $("#assistance_difference").html(
      (
        Math.round(
          ((assistance_payback - value * assistance_ratio) / 100) * 100
        ) / 100
      ).toFixed(2)
    );
    $("#assistance_calculated").html(
      Math.round(value * (assistance_ratio / 100)).toFixed(2)
    );
  }
